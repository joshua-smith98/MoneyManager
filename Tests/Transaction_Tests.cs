...
namespace Tests
{
    ...
    public class Transaction_Tests
    {
        ...
        {
            // Transaction type must be 'Withdrawal' for negative values, 'Deposit' for positive values, and 'Null' for 0
            // Arrange
            var nullT = new Transaction(0, "");
            var withdrawalT = new Transaction(-10, "");
            var depositT = new Transaction(10, "");

            // Act and Assert
            Assert.IsTrue(nullT.TransactionType is TransactionType.Null);
